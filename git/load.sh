...
git config --global --replace-all alias.rup '! f() { git remote update --prune; for orphaned_branch in $(git branch -vv --format "%(refname)|%(upstream:track)" | grep "|\[gone\]" | sed -E "s/refs\/heads\/([^|]*)\|\[gone\]/\1/"); do echo "Deleting orhpaned branch $orphaned_branch"; git branch -D $orphaned_branch; done; }; f'
